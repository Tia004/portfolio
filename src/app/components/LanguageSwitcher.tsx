...
export default function LanguageSwitcher({ variant }: { variant?: 'desktop' | 'clean' }) {
  ...
  // Clean (mobile hamburger & footer) variant — ultra clean, no borders, no background
  if (isClean) {
    return (
          ...
          <div className="absolute top-full mt-1.5 left-0 z-50 flex flex-col items-start gap-1 shrink-0">
            {Object.keys(FULL_NAMES).filter(code => code !== lang).map((code) => (
              <button
                ...
                className="whitespace-nowrap text-sm font-medium transition-colors text-white/50 hover:text-teal-400 cursor-pointer"
              >
                {FULL_NAMES[code]}
              </button>
            ))}
        ...
        <div className="absolute top-full mt-1.5 right-0 bg-[#121212]/95 backdrop-blur-xl border border-white/10 rounded-xl p-1 shadow-2xl shadow-black/60 z-50 min-w-[100px]">
          {Object.keys(SHORT_NAMES).filter(code => code !== lang).map((code) => (
            <button
              key={code}
              onClick={() => handleSelectLang(code)}
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium transition-all text-white/80 hover:bg-white/[0.08] hover:text-white cursor-pointer"
            >
